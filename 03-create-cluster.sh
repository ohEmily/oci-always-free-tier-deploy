#!/bin/bash
# 03-create-cluster.sh - Create OKE cluster control plane
#
...
#
# Use this with 04-create-nodepool.sh when dealing with
...
main() {
  print_section "Step 3: Create OKE Cluster (Control Plane Only)"
  ...
  explain "This creates only the OKE control plane (API server, etcd, etc.).
   ...
   Node pool creation is handled separately by 04-create-nodepool.sh"
  ...
  echo ""
  echo "Next step: Run ./04-create-nodepool.sh"
